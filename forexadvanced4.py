...
# ===========================
CONFIG = {
    "MODE": "aggressive",               # safe / proactive / aggressive
    "SYMBOL": "EURUSD",
...
def build_default_deep(input_shape, lr=1e-3):
    # simple Conv1D model for sequences (input_shape = (seq_len, n_features))
...
def place_trade(signal, df_raw, rf_model=None, deep_model=None):
    ...
    # determine filling mode safely
    fill_mode = mt5.ORDER_FILLING_FOK
    try:
        if sym is not None:
            if hasattr(sym, "filling_mode"):
                fill_mode = sym.filling_mode
            elif hasattr(sym, "trade_fill_mode"):
                fill_mode = sym.trade_fill_mode
    except Exception:
        fill_mode = mt5.ORDER_FILLING_FOK
